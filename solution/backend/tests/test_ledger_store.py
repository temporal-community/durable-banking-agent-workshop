...
def test_concurrent_transfers_do_not_lose_updates(isolated_ledger):
    ...
    file. Without the fcntl lock added alongside this test, firing many concurrent transfers from
    B (which only receives money here, never spends it, so none of them can fail on insufficient
    funds) reliably loses updates - the final balance ends up short of the expected total because
    two threads read the same starting balance before either wrote back. With the lock, every
    credit lands."""
    N = 25
